"""
core/audio_devices.py — pick which microphone and which speakers ARC uses.
...
WHY
    ...
    "ARC can't hear me" almost always means "ARC is listening to the
...
def _usable(idx: int, kind: str) -> bool:
    ...
    Tests both mono (1 channel) and stereo (2 channels) since Bluetooth
    headsets on Windows often reject 1-channel output while accepting 2-channel."""
    st = None
    try:
        import sounddevice as sd
        rate = _RATES.get(kind, 16000)
        if kind == "input":
            try:
                ...
                st = sd.InputStream(samplerate=rate, channels=2, dtype="int16",
                                    blocksize=1024, device=idx,
                                    callback=lambda *_a: None)
        else:
            try:
