...
def _play_np(samples, sample_rate: int) -> None:
    """Play float32 mono (or stereo) audio via sounddevice with volume boost normalization."""
    arr = _to_numpy(samples)
    if len(arr) > 0:
        max_val = float(np.max(np.abs(arr)))
        if max_val > 0.01 and max_val < 0.85:
            gain = min(2.5, 0.95 / max_val)
...
def _play_audio_bytes(audio_bytes: bytes) -> None:
    """Decode MP3/WAV/OGG bytes and play via sounddevice with volume boost normalization."""
    import miniaudio
