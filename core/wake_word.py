"""
Local wake-word detection for ARC ("Hey Arc").
...
CPU. The pretrained wake phrase used here is "Hey Arc".
"""
...
# Pretrained openwakeword model that listens for "Hey Arc".
...
class WakeWordDetector:
    ...
    def __init__(
        ...
        biometric_threshold: float = 0.80,
    ):
        import collections
        ...
        self._verify_speaker = verify_speaker
        self._biometric_threshold = biometric_threshold
        self._recent_audio = collections.deque(maxlen=20)  # ~1.5-2 seconds of frames
    ...
    def set_playback_checker(self, fn: Callable[[], bool]) -> None:
        self._is_playback_active = fn

    def set_speaker_verification(self, enabled: bool, threshold: float = 0.80) -> None:
        self._verify_speaker = enabled
        self._biometric_threshold = threshold
    ...
    def start(self) -> bool:
        ...
        self._thread.start()
        self._logger("Wake word: listening for 'Hey Arc'.")
        return True
        ...
        while self._running:
            try:
                ...
                if score >= self._threshold:
                    # 1. Self-playback suppression: don't wake if assistant audio is actively playing
                    if self._is_playback_active and self._is_playback_active():
                        self._logger("Wake word: suppressed (self-wake prevention — assistant is speaking).")
                        self._drain()
                        continue

                    # 2. Speaker Diarization / Biometric check
                    if self._verify_speaker:
                        try:
                            from core.confirm import verify_speaker_voice, is_speaker_enrolled
                            if is_speaker_enrolled() and len(self._recent_audio) > 0:
                                recent_buf = np.concatenate(list(self._recent_audio))
                                if not verify_speaker_voice(recent_buf, threshold=self._biometric_threshold):
                                    self._logger("Wake word: suppressed (speaker diarization — unrecognized voice).")
                                    self._drain()
                                    continue
