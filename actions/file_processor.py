"""
file_processor.py — ARC Universal File Processor
...
def _process_pptx(path: Path, action: str, params: dict, speak=None) -> str:
    ...
    def _read_pptx_text() -> str:
        ...
        # Fallback: Parse XML from .pptx zip archive directly
        try:
            import zipfile
...
def _chunk_text(text: str, chunk_size: int = 2000, overlap: int = 150) -> list[str]:
    chunks = []
    start = 0
    clean = text.strip()
...
def ingest_document_to_semantic_store(path: Path, player=None) -> str:
    """Ingest PDF, DOCX, PPTX, or TXT into semantic memory chunks (~512 tokens)."""
    ext = path.suffix.lower().lstrip(".")
    text = ""
            ...
            try:
                ...
                with zipfile.ZipFile(str(path), "r") as z:
                    ...
                    for s_file in slide_files:
                        ...
                        texts.extend([elem.text for elem in tree.iter() if elem.text and elem.text.strip()])
                text = "\n\n".join(texts)
            except Exception:
                pass
    ...
    if not text or len(text.strip()) < 30:
        return f"Could not extract sufficient text from '{path.name}' to ingest."

    chunks = _chunk_text(text, chunk_size=2000, overlap=150)
    try:
        from memory.memory_manager import add_semantic_memory
        for idx, chunk in enumerate(chunks, 1):
            concept = f"doc_{re.sub(r'[^a-zA-Z0-9]', '_', path.stem)[:20]}_part_{idx}".lower()
            add_semantic_memory(
                lesson=chunk,
                concept=concept,
                mem_type="document_chunk",
                source=f"document:{path.name}",
                priority=7,
                context=f"Ingested from document '{path.name}', section {idx}/{len(chunks)}",
            )
        msg = f"Successfully ingested '{path.name}' into semantic memory ({len(chunks)} chunks, {len(text):,} chars). ARC can now recall and answer questions about this document."
        print(f"[FileProcessor] {msg}")
...
def file_processor(parameters: dict, player=None, speak=None) -> str:
    ...
    # Ingestion / indexing action
    if action in ("ingest", "index", "embed", "learn"):
        return ingest_document_to_semantic_store(path, player=player)
