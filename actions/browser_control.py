    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using ARC profile")
        ...
        # automation). Fall back to a persistent ARC automation profile —
        ...
        print(f"[Browser] Retrying with ARC profile: {jarvis_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(jarvis_profile, **kwargs)
            self._page = await self._adopt_page()
            print(f"[Browser] ✅ Launched [{label}] with ARC profile "
