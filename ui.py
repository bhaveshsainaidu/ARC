...
# ── Force Discrete NVIDIA GPU & Direct3D 11 RHI for smooth rendering ────────
...
# ── Product identity ─────────────────────────────────────────────────────────
# These values are used by the window title, header badge, and HUD panel.  The
# assistant's spoken name remains independently configurable in Settings.
APP_VERSION  = "Adaptive Real-time Cognitive Agent"
APP_PROTOCOL = "CORE"
...
class _SysMetrics:
    ...
    def _update(self):
        ...
        vram_used = getattr(self, "vram_used_mb", 0.0)
        vram_total = getattr(self, "vram_total_mb", 0.0)

        self._slow_tick = (self._slow_tick + 1) % 3
        if self._slow_tick == 1:
            try:
                from core.gpu_accelerator import get_vram_telemetry
                vtel = get_vram_telemetry()
                if vtel.get("active"):
                    gpu = vtel.get("gpu_core_util_pct", self.gpu)
                    vram_used = vtel.get("vram_used_mb", 0.0)
                    vram_total = vtel.get("vram_total_mb", 0.0)
                    vram_pct = vtel.get("vram_util_pct", 0.0)
                    g_temp = vtel.get("temperature_c", -1.0)
                    tmp = g_temp if g_temp > 0 else self._get_temp()
                else:
                    gpu = self._get_gpu()
                    tmp = self._get_temp()
            except Exception:
...
class HudCanvas(QWidget):
    def __init__(self, face_path: str, assistant_name: str = "J.A.R.V.I.S", parent=None):
        ...
        # ── Golden Energy Sphere 3D geometry (Optimized) ──────────────────────
        self._num_sphere_pts = 72
        self._sphere_pts_3d: list[tuple[float, float, float]] = []
        phi = math.pi * (3.0 - math.sqrt(5.0))  # golden ratio angle (~2.39996 rad)
        for i in range(self._num_sphere_pts):
            y = 1.0 - (i / float(self._num_sphere_pts - 1)) * 2.0
            rad = math.sqrt(max(0.0, 1.0 - y * y))
            theta = phi * i
            x = math.cos(theta) * rad
            z = math.sin(theta) * rad
            self._sphere_pts_3d.append((x, y, z))

        # Precompute geodesic lines (3D unit sphere distance < 0.34, capped at 40)
        self._geodesic_edges: list[tuple[int, int]] = []
        _thresh_sq = 0.34 * 0.34
        for i in range(self._num_sphere_pts):
            xi, yi, zi = self._sphere_pts_3d[i]
            for j in range(i + 1, self._num_sphere_pts):
                xj, yj, zj = self._sphere_pts_3d[j]
                dsq = (xi - xj) ** 2 + (yi - yj) ** 2 + (zi - zj) ** 2
                if dsq < _thresh_sq:
                    self._geodesic_edges.append((i, j))
        if len(self._geodesic_edges) > 40:
            self._geodesic_edges = self._geodesic_edges[:40]

        self._rot_x = 0.2
        self._rot_y = 0.0
        self._orbit_angles = [0.0, 120.0, 240.0]
        self._sparks: list[list[float]] = []  # [x, y, vx, vy, life, max_life, r, g, b]

        # ── Cyber Shield Threat Visualizer State ──────────────────────────────
        self._threat_level = "ALL_CLEAR"  # "CRITICAL" | "HIGH" | "MEDIUM" | "ALL_CLEAR" | "OFFLINE"
        self._threat_color = [255.0, 215.0, 0.0]  # Smooth RGB transition
    ...
    def set_threat_level(self, level: str) -> None:
        ...
        self._threat_level = str(level).upper().strip()
    ...
    def _step(self):
        ...
        # Voice Reactivity Speeds & Breathing:
        # SLEEPING: Sphere rotates slowly (0.5x speed), dim gold, slow breathing pulsation (2s period)
        # LISTENING: Outer ring expands and pulses, wave ripple outward from core, bright gold #FFD700. Reacts to mic RMS volume.
        # THINKING: Particles accelerate rotation (2.5x speed), rings spin rapidly in opposite directions, color shifts toward bright white-gold.
        # SPEAKING: Core pulses strongly in sync with assistant audio output level, spark bursts erupt outward, energy rings ripple.
        if self.state == "SLEEPING":
            rot_mult = 0.5
            breathe_rate = 0.045  # ~2.0s period
            self._base_scale = 1.0 + 0.03 * math.sin(self._tick * breathe_rate)
            self._base_halo  = 30.0 + 10.0 * math.sin(self._tick * breathe_rate)
        elif self.state == "THINKING":
            rot_mult = 2.5
            breathe_rate = 0.15
            self._base_scale = 1.05 + 0.05 * math.sin(self._tick * breathe_rate)
            self._base_halo  = 110.0
        elif self.state == "LISTENING":
            rot_mult = 1.0 + amp * 1.5
            breathe_rate = 0.09
            self._base_scale = 1.02 + 0.03 * math.sin(self._tick * breathe_rate)
            self._base_halo  = 65.0 + amp * 50.0
        elif self.speaking:
            rot_mult = 1.6 + amp * 2.2
            breathe_rate = 0.18
            self._base_scale = 1.06 + amp * 0.15
            self._base_halo  = 120.0 + amp * 80.0
        else:
            rot_mult = 0.8
            breathe_rate = 0.07
            self._base_scale = 1.01 + 0.02 * math.sin(self._tick * breathe_rate)
            self._base_halo  = 50.0

        self._rot_y = (self._rot_y + 0.012 * rot_mult) % (2 * math.pi)
        self._rot_x = (self._rot_x + 0.006 * rot_mult) % (2 * math.pi)

        # Orbiting ring speeds (counter-rotating in THINKING):
        if self.state == "THINKING":
            ring_spds = [3.5, -4.2, 3.0]
        elif self.speaking:
            ring_spds = [1.8 + amp * 2.5, -1.5 - amp * 2.0, 2.2 + amp * 3.0]
        elif self.state == "LISTENING":
            ring_spds = [1.1 + amp * 1.5, -0.9 - amp * 1.2, 1.4 + amp * 2.0]
        elif self.state == "SLEEPING":
            ring_spds = [0.4, -0.3, 0.5]
        else:
            ring_spds = [0.8, -0.6, 1.0]

        for i, spd in enumerate(ring_spds):
            self._orbit_angles[i] = (self._orbit_angles[i] + spd) % 360.0

        if self.muted:
            self._tgt_scale, self._tgt_halo = self._base_scale, 20.0
        elif self.speaking:
        ...
        # ── Smooth Threat Color Interpolation (0.5s transition) ───────────────
        if self._threat_level == "CRITICAL":
            target_rgb = [255.0, 35.0, 55.0]
        elif self._threat_level == "HIGH":
            target_rgb = [255.0, 140.0, 0.0]
        elif self._threat_level in ("MEDIUM", "WARNING"):
            target_rgb = [255.0, 215.0, 0.0]
        else:
            target_rgb = [255.0, 215.0, 0.0]

        for idx in range(3):
            self._threat_color[idx] += (target_rgb[idx] - self._threat_color[idx]) * 0.12

        # ── Outer spark burst generation (Capped at 12 sparks for low latency) ─
        fw = min(self.width(), self.height())
        cx, cy = self.width() / 2, self.height() / 2
        base_r = fw * 0.23 * self._scale
        spark_prob = 0.06
        if self.speaking or amp > 0.04:
            spark_prob = 0.25 + amp * 0.35
        elif self.state == "THINKING" or self._threat_level == "CRITICAL":
            spark_prob = 0.20

        if random.random() < spark_prob and len(self._sparks) < 12:
            s_ang = random.uniform(0, 2 * math.pi)
            s_spd = random.uniform(1.2, 3.5) if (self.speaking or amp > 0.05) else random.uniform(0.8, 1.8)
            if self._threat_level == "CRITICAL":
                col_pick = (255, 60, 80)
            elif self._threat_level == "HIGH":
                col_pick = (255, 140, 0)
            else:
                col_pick = random.choice([
                    (255, 215, 0),   # #FFD700
                    (255, 165, 0),   # #FFA500
                    (255, 228, 181), # #FFE4B5
                    (255, 140, 0),   # #FF8C00
                ])
            max_l = random.randint(20, 35)
            self._sparks.append([
                cx + math.cos(s_ang) * base_r,
                cy + math.sin(s_ang) * base_r,
                math.cos(s_ang) * s_spd,
                math.sin(s_ang) * s_spd,
                float(max_l),
                float(max_l),
                col_pick[0], col_pick[1], col_pick[2],
            ])

        # Decay sparks
        self._sparks = [
            [s[0] + s[2], s[1] + s[3], s[2] * 0.96, s[3] * 0.96, s[4] - 1.0, s[5], s[6], s[7], s[8]]
            for s in self._sparks if s[4] > 1.0
        ]

        # Wave ripples / pulses (Rapid pulse if CRITICAL)
        lim = fw * 0.74
        if self._threat_level == "CRITICAL":
            p_spd = 6.5
        else:
            p_spd = 4.2 if self.speaking else 2.0
        self._pulses = [r + p_spd for r in self._pulses if r + p_spd < lim]
        pulse_limit = 4 if self._threat_level == "CRITICAL" else 3
        if len(self._pulses) < pulse_limit and random.random() < (0.15 if self._threat_level == "CRITICAL" else (0.07 if self.speaking else 0.025)):
    ...
    def paintEvent(self, _):
        ...
        # ── 1. Core Energy Glow (Radial Gradient with Threat Shift) ──────────
        glow_r = R * (1.35 + amp * 0.3)
        rad_grad = QRadialGradient(QPointF(cx, cy), glow_r)
        cr, cg, cb = int(self._threat_color[0]), int(self._threat_color[1]), int(self._threat_color[2])

        if self.muted:
            c_alpha = 90
            rad_grad.setColorAt(0.0, QColor(255, 60, 80, c_alpha))
            rad_grad.setColorAt(0.4, QColor(200, 30, 50, int(c_alpha * 0.7)))
            rad_grad.setColorAt(0.7, QColor(140, 10, 30, int(c_alpha * 0.4)))
            rad_grad.setColorAt(1.0, QColor(100, 0, 20, 0))
        elif self._threat_level == "CRITICAL":
            c_alpha = 240
            rad_grad.setColorAt(0.0, QColor(255, 120, 120, c_alpha))
            rad_grad.setColorAt(0.35, QColor(255, 35, 55, int(c_alpha * 0.85)))
            rad_grad.setColorAt(0.70, QColor(180, 0, 25, int(c_alpha * 0.50)))
            rad_grad.setColorAt(1.00, QColor(100, 0, 10, 0))
        elif self._threat_level == "HIGH":
            c_alpha = 220
            rad_grad.setColorAt(0.0, QColor(255, 200, 120, c_alpha))
            rad_grad.setColorAt(0.35, QColor(255, 140, 0, int(c_alpha * 0.85)))
            rad_grad.setColorAt(0.70, QColor(200, 80, 0, int(c_alpha * 0.50)))
            rad_grad.setColorAt(1.00, QColor(150, 40, 0, 0))
        elif self.state == "SLEEPING":
            c_alpha = int(self._halo * 1.5)
            rad_grad.setColorAt(0.0, QColor(cr, cg, cb, c_alpha))
            rad_grad.setColorAt(0.4, QColor(cr, cg, cb, int(c_alpha * 0.75)))
            rad_grad.setColorAt(0.7, QColor(max(0, cr - 80), max(0, cg - 60), 0, int(c_alpha * 0.40)))
            rad_grad.setColorAt(1.0, QColor(max(0, cr - 80), max(0, cg - 60), 0, 0))
        elif self.state == "THINKING":
            c_alpha = 255
            rad_grad.setColorAt(0.0, QColor(255, 255, 255, c_alpha))
            rad_grad.setColorAt(0.35, QColor(255, 248, 220, 240))
            rad_grad.setColorAt(0.65, QColor(255, 215, 0, 160))
            rad_grad.setColorAt(1.0, QColor(255, 140, 0, 0))
        else:
            c_alpha = min(255, int(180 + amp * 75))
            rad_grad.setColorAt(0.0, QColor(min(255, cr + 40), min(255, cg + 40), min(255, cb + 40), c_alpha))
            rad_grad.setColorAt(0.40, QColor(cr, cg, cb, int(c_alpha * 0.82)))
            rad_grad.setColorAt(0.70, QColor(max(0, cr - 60), max(0, cg - 60), 0, int(c_alpha * 0.50)))
            rad_grad.setColorAt(1.00, QColor(max(0, cr - 90), max(0, cg - 90), 0, 0))
        ...
        proj_pts: list[tuple[float, float, float]] = []  # (px, py, depth_norm)
        for x, y, z in self._sphere_pts_3d:
            y1 = y * cos_x - z * sin_x
            z1 = y * sin_x + z * cos_x
            x2 = x * cos_y + z1 * sin_y
            z2 = -x * sin_y + z1 * cos_y
            px = cx + x2 * R
            py = cy + y1 * R
            d_norm = (z2 + 1.0) * 0.5  # depth in [0, 1]
            proj_pts.append((px, py, d_norm))

        # ── 4. Geodesic / Laser Grid Lines (Batched drawLines) ───────────────
        lines_to_draw: list[QLineF] = []
        for i, j in self._geodesic_edges:
            px_i, py_i, di = proj_pts[i]
            px_j, py_j, dj = proj_pts[j]
            d_avg = (di + dj) * 0.5
            if d_avg > 0.22:
                lines_to_draw.append(QLineF(px_i, py_i, px_j, py_j))

        if lines_to_draw:
            if self._threat_level == "CRITICAL":
                line_pen = QPen(QColor(255, 80, 80, 170), 1.0)
            elif self._threat_level == "HIGH":
                line_pen = QPen(QColor(255, 160, 50, 170), 1.0)
            else:
                line_alpha = 150 if self.state != "SLEEPING" else 60
                line_pen = QPen(QColor(255, 228, 181, line_alpha), 1.0)
            p.setPen(line_pen)
            p.drawLines(lines_to_draw)

        # ── 2. Rotating 3D Particle Sphere (Depth Bucketed) ──────────────────
        p.setPen(Qt.PenStyle.NoPen)
        front_pts: list[tuple[float, float, float]] = []
        mid_pts: list[tuple[float, float, float]] = []
        back_pts: list[tuple[float, float, float]] = []

        for px, py, d in proj_pts:
            sz = 1.0 + d * 2.8
            if d > 0.65:
                front_pts.append((px, py, sz))
            elif d > 0.35:
                mid_pts.append((px, py, sz))
            else:
                back_pts.append((px, py, sz))

        if back_pts:
            p.setBrush(QBrush(QColor(max(0, cr - 80), max(0, cg - 80), max(0, cb - 80), 90)))
            for px, py, sz in back_pts:
                p.drawEllipse(QPointF(px, py), sz, sz)
        if mid_pts:
            p.setBrush(QBrush(QColor(cr, cg, cb, 175)))
            for px, py, sz in mid_pts:
                p.drawEllipse(QPointF(px, py), sz, sz)
        if front_pts:
            front_col = QColor(255, 255, 255, 240) if self.state == "THINKING" else QColor(min(255, cr + 40), min(255, cg + 40), min(255, cb + 40), 230)
            p.setBrush(QBrush(front_col))
            for px, py, sz in front_pts:
                p.drawEllipse(QPointF(px, py), sz, sz)

        # ── 3. Orbiting Energy Rings (3 rings tilted at 30°, 60°, -45°) ──────
        ring_tilts = [30.0, 60.0, -45.0]
        ring_colors = [
            (255, 215, 0),   # #FFD700
            (255, 165, 0),   # #FFA500
            (255, 228, 181), # #FFE4B5
        ]
        for idx, tilt in enumerate(ring_tilts):
            p.save()
            p.translate(cx, cy)
            p.rotate(tilt)
            rx = R * (1.45 + idx * 0.18)
            ...
            base_ang = self._orbit_angles[idx]
            rc = ring_colors[idx]
            ring_a = 180 if self.state != "SLEEPING" else 75
            if self.speaking or self.state == "THINKING":
                ring_a = min(255, int(ring_a * 1.3))

            p.setPen(QPen(QColor(rc[0], rc[1], rc[2], ring_a), 1.6, Qt.PenStyle.DashLine))
            p.setBrush(Qt.BrushStyle.NoBrush)
            p.drawEllipse(QRectF(-rx, -ry, rx * 2, ry * 2))

            # Orbiting energy bead
            bead_rad = math.radians(base_ang)
            bx = rx * math.cos(bead_rad)
            by = ry * math.sin(bead_rad)
            p.setPen(Qt.PenStyle.NoPen)
            p.setBrush(QBrush(QColor(255, 255, 255, min(255, ring_a + 60))))
            p.drawEllipse(QPointF(bx, by), 3.2, 3.2)
            p.restore()

        # ── 5. Outer Spark Burst ─────────────────────────────────────────────
        for spk in self._sparks:
            sx, sy, _, _, life, max_life, sr, sg, sb = spk
            life_frac = max(0.0, life / max_life)
            spk_a = int(255 * life_frac)
            spk_sz = 1.5 + life_frac * 2.2
            p.setPen(Qt.PenStyle.NoPen)
            p.setBrush(QBrush(QColor(int(sr), int(sg), int(sb), spk_a)))
            p.drawEllipse(QPointF(sx, sy), spk_sz, spk_sz)

        # ── Scanners & Reticle Elements ──────────────────────────────────────
        sr = fw * 0.50
        ...
        disp_name = "ARC" if self._assistant_name in ("ARC", "J.A.R.V.I.S", "ARC") else self._assistant_name
        p.drawText(QRectF(cx - 80, cy - 14, 160, 28), Qt.AlignmentFlag.AlignCenter, disp_name)

        if self._threat_level == "CRITICAL":
            p.setPen(QPen(QColor(255, 50, 60, 240), 1))
            p.setFont(QFont("Courier New", 10, QFont.Weight.Bold))
            p.drawText(QRectF(cx - 110, cy + 18, 220, 24), Qt.AlignmentFlag.AlignCenter, "⚠ THREAT DETECTED")

            # Flashing red perimeter boundary
            p.setPen(QPen(QColor(255, 40, 50, 210 if self._blink else 90), 2.0))
            p.setBrush(Qt.BrushStyle.NoBrush)
        ...
        # ── Audio Waveform (Golden / Amber Spectrum) ─────────────────────────
        wy = sy + 30
        N, bw = 36, 8
        wx0 = (W - N * bw) / 2
        mid = (N - 1) / 2.0
        for i in range(N):
            if self.muted:
                ...
                if amp > 0.05:
                    cl = qcol(C.PRI) if hgt > 12 else qcol(C.ACC2)
                else:
                    cl = qcol(C.BORDER_B)
...
class LogWidget(QTextEdit):
    ...
    def __init__(self, parent=None):
        ...
        self._pos     = 0
        self._tag     = "sys"
        self._ai_name_lc = "arc"   # updated when assistant name changes
        self._tmr = QTimer(self)
        self._tmr.timeout.connect(self._step)
    ...
    def _get_tag(self, text: str) -> str:
        tl = text.lower()
        _ai_pfx = f"{self._ai_name_lc}:"
        if   tl.startswith("you:"):                               return "you"
        elif tl.startswith(_ai_pfx) or tl.startswith("arc:"):     return "ai"
        elif tl.startswith("file:"):                              return "file"
        elif "threat" in tl or tl.startswith("⚠"):               return "threat"
        elif tl.startswith("security:") or "security:" in tl:     return "security"
        elif tl.startswith("blocked:") or "blocked:" in tl:       return "blocked"
        elif "err" in tl:                                         return "err"
        else:                                                     return "sys"

    def _insert_line_direct(self, text: str):
    ...
    def _enqueue(self, text: str):
        tl = text.lower().strip()
        # Suppress routine background noise from popping up on HUD
        if "low anomaly noted" in tl:
            return
        if "detected:" in tl and "[emotion]" in tl:
            return
        if ("threat:" in tl or "critical threat" in tl) and not text.startswith("⚠"):
            text = f"⚠ {text}"

        # If backlog exists, flush older queued items directly so user never waits on queued typing
        if len(self._queue) >= 2:
            while len(self._queue) > 0:
                self._insert_line_direct(self._queue.pop(0))
            self._insert_line_direct(text)
            self._typing = False
            return
    ...
    def _next(self):
        ...
        self._pos    = 0
        self._tag    = self._get_tag(self._text)
        self._tmr.start(6)
...
class FileDropZone(QWidget):
    ...
    def _browse(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Select a file for ARC", str(Path.home()),
            "All Files (*.*);;"
...
class CustomizeOverlay(QWidget):
    ...
    def __init__(self, assistant_name="ARC", user_name="",
    ...
    def _save(self):
        name = self._name_input.text().strip() or "ARC"
...
class AudioDeviceOverlay(_HudOverlay):
    """Choose which microphone ARC listens to and which speakers it uses.
    ...
    is plugged in. 'ARC can't hear me' is usually 'ARC is listening to the
    webcam'."""
    ...
    def __init__(self, parent=None):
        ...
        from memory.config_manager import (
            ...
            get_barge_in_sensitivity, get_headset_mode, get_ptt_enabled,
        )
        ...
        def _row(label: str, kind: str, current: str) -> QComboBox:
            ...
            try:
                dev_list = list_devices(kind, refresh=True)
            except Exception:
                dev_list = list_devices(kind, refresh=False)
        ...
        self._in_box  = _row("MICROPHONE — what ARC hears you with",
                             "input", get_input_device())
        lay.addSpacing(4)
        self._out_box = _row("SPEAKERS — what ARC talks through",
                             "output", get_output_device())

        lay.addSpacing(6)
        v_hdr = QLabel("INTERRUPTION & BARGE-IN")
        v_hdr.setFont(QFont("Courier New", 8, QFont.Weight.Bold))
        v_hdr.setStyleSheet(f"color: {C.ACC2}; background: transparent;")
        lay.addWidget(v_hdr)

        # Barge-in sensitivity
        self._sens_box = QComboBox()
        self._sens_box.setFont(QFont("Courier New", 9))
        self._sens_box.setFixedHeight(30)
        self._sens_box.setStyleSheet(_combo_css)
        self._sens_box.addItem("Low (Noisy room / Speaker heavy)", "low")
        self._sens_box.addItem("Medium (Balanced default)", "medium")
        self._sens_box.addItem("High (Quiet room / Fast barge-in)", "high")
        cur_sens = get_barge_in_sensitivity()
        s_idx = self._sens_box.findData(cur_sens)
        self._sens_box.setCurrentIndex(s_idx if s_idx >= 0 else 1)
        lay.addWidget(self._sens_box)

        # Checkboxes
        from PyQt6.QtWidgets import QCheckBox
        cb_css = f"QCheckBox {{ color: {C.TEXT_MED}; font-family: 'Courier New'; font-size: 8pt; }}"
        self._headset_cb = QCheckBox("Headset Mode (Headphones on — disable speaker echo filter)")
        self._headset_cb.setStyleSheet(cb_css)
        self._headset_cb.setChecked(get_headset_mode())
        lay.addWidget(self._headset_cb)

        self._ptt_cb = QCheckBox("Push-to-Talk Mode (Hold Spacebar to speak)")
        self._ptt_cb.setStyleSheet(cb_css)
        self._ptt_cb.setChecked(get_ptt_enabled())
        lay.addWidget(self._ptt_cb)

        # Calibration button & readout
        cal_row = QHBoxLayout(); cal_row.setSpacing(6)
        self._cal_btn = QPushButton("⚡  CALIBRATE NOISE FLOOR")
        self._cal_btn.setFixedHeight(26)
        self._cal_btn.setFont(QFont("Courier New", 8))
        self._cal_btn.setCursor(Qt.CursorShape.PointingHandCursor)
        self._cal_btn.setStyleSheet(f"""
            QPushButton {{ background: #00121a; color: {C.ACC2};
        ...
        self._cal_lbl = QLabel("Room noise: unmeasured")
        self._cal_lbl.setFont(QFont("Courier New", 7))
        self._cal_lbl.setStyleSheet(f"color: {C.TEXT_DIM}; background: transparent;")
        cal_row.addWidget(self._cal_lbl)
        lay.addLayout(cal_row)

        note = QLabel("Applying updates voice settings immediately.")
    ...
    def _run_calibration(self):
        """Sample microphone for 1.2 seconds to determine ambient room noise floor."""
        import sounddevice as sd
        import numpy as np
        from core.audio_devices import resolve
        from memory.config_manager import get_input_device

        self._cal_lbl.setText("Sampling room noise (1.2s)...")
        self._cal_btn.setEnabled(False)

        def worker():
            try:
                mic_name = self._in_box.currentData() if hasattr(self, "_in_box") else get_input_device()
                dev_idx = resolve(mic_name, "input")
                samples = None
                try:
                    samples = sd.rec(int(1.2 * 16000), samplerate=16000, channels=1, dtype="int16", device=dev_idx)
                    sd.wait()
                except Exception:
                    samples = sd.rec(int(1.2 * 16000), samplerate=16000, channels=2, dtype="int16", device=dev_idx)
                    sd.wait()
                x = np.asarray(samples, dtype=np.float32)
                rms = float(np.sqrt(np.mean(x * x)))
                if rms < 90:
                    rec = "high"
                    rec_name = "High (Quiet room)"
                elif rms <= 220:
                    rec = "medium"
                    rec_name = "Medium (Balanced)"
                else:
                    rec = "low"
                    rec_name = "Low (Noisy room)"

                def finish():
                    self._cal_lbl.setText(f"RMS: {rms:.0f} → {rec_name}")
                    idx = self._sens_box.findData(rec)
                    if idx >= 0:
                        self._sens_box.setCurrentIndex(idx)
                    self._cal_btn.setEnabled(True)

                QTimer.singleShot(0, finish)
            except Exception as e:
                def on_err():
                    self._cal_lbl.setText(f"Calibration failed: {e}")
                    self._cal_btn.setEnabled(True)
    ...
    def _apply(self):
        from memory.config_manager import (
            ...
            save_barge_in_sensitivity, save_headset_mode, save_ptt_enabled,
        )
        new_in  = self._in_box.currentData()  or ""
        new_out = self._out_box.currentData() or ""
        changed_devices = (new_in != get_input_device()) or (new_out != get_output_device())
        save_input_device(new_in)
        save_output_device(new_out)

        # Save voice settings
        new_sens = self._sens_box.currentData() or "medium"
        save_barge_in_sensitivity(new_sens)
        save_headset_mode(self._headset_cb.isChecked())
        save_ptt_enabled(self._ptt_cb.isChecked())

        # Notify parent window if attached
        p = self.parent()
        while p and not hasattr(p, "on_voice_settings_change"):
            p = p.parent()
        if p and hasattr(p, "on_voice_settings_change") and p.on_voice_settings_change:
            try:
                p.on_voice_settings_change()
            except Exception:
                pass
...
class IncidentTimelineOverlay(_HudOverlay):
    """Floating overlay panel displaying ARC Cyber Shield incidents timeline."""
    ...
    def __init__(self, parent=None):
        ...
        self.setAttribute(Qt.WidgetAttribute.WA_StyledBackground, True)
        self.setStyleSheet(f"""
            IncidentTimelineOverlay {{
                background: rgba(10, 8, 0, 250);
    ...
    def _rebuild(self):
        ...
        hdr_row = QHBoxLayout()
        hdr = QLabel("🛡  ARC CYBER SHIELD — INCIDENT TIMELINE")
        hdr.setFont(QFont("Courier New", 11, QFont.Weight.Bold))
        hdr.setStyleSheet(f"color: {C.PRI}; background: transparent;")
        ...
        btn_refresh.setStyleSheet(f"background: transparent; color: {C.TEXT_MED}; border: 1px solid {C.BORDER}; border-radius: 3px; padding: 0 6px;")
        btn_refresh.clicked.connect(self._rebuild)
        hdr_row.addWidget(btn_refresh)

        btn_close = QPushButton("✕")
        btn_close.setFixedSize(26, 26)
        btn_close.setFont(QFont("Courier New", 9, QFont.Weight.Bold))
        btn_close.setStyleSheet(f"background: transparent; color: {C.TEXT_MED}; border: 1px solid {C.BORDER}; border-radius: 3px;")
        ...
        incidents = []
        try:
            from plugins.threat_monitor import INCIDENTS_DIR
            if INCIDENTS_DIR.exists():
                for f in sorted(INCIDENTS_DIR.glob("incident_*.md"), reverse=True)[:35]:
                    try:
                        txt = f.read_text(encoding="utf-8")
                        m_sev = re.search(r"\*\*Severity:\*\*\s*(\w+)", txt)
                        m_type = re.search(r"\*\*Type:\*\*\s*(\w+)", txt)
                        m_time = re.search(r"\*\*Timestamp:\*\*\s*([^\n]+)", txt)
                        m_sum = re.search(r"## Threat Summary\s*\n+([^\n]+)", txt)
                        incidents.append({
                            "severity": m_sev.group(1) if m_sev else "HIGH",
                            "type": m_type.group(1) if m_type else "Incident",
                            "timestamp": m_time.group(1) if m_time else f.stem.replace("incident_", ""),
                            "summary": m_sum.group(1) if m_sum else "Security finding recorded.",
                            "file": f.name,
                        })
                    except Exception:
                        continue
        ...
        stats_lbl = QLabel(
            f"Surveillance History: {len(incidents)} Incidents | "
            f"Critical: {sev_counts['CRITICAL']} | High: {sev_counts['HIGH']} | "
            f"Medium: {sev_counts['MEDIUM']} | Low: {sev_counts['LOW']}"
        )
        stats_lbl.setFont(QFont("Courier New", 7))
        stats_lbl.setStyleSheet(f"color: {C.TEXT_MED}; background: transparent; padding: 2px 0;")
        self._lay.addWidget(stats_lbl)

        scroll = QScrollArea()
        scroll.setWidgetResizable(True)
        scroll.setStyleSheet(f"background: transparent; border: 1px solid {C.BORDER};")
        s_content = QWidget()
        ...
        if not incidents:
            empty_lbl = QLabel("✓ No security incidents recorded. All perimeters secure.")
            empty_lbl.setFont(QFont("Courier New", 8))
            empty_lbl.setStyleSheet(f"color: {C.GREEN}; padding: 30px;")
            empty_lbl.setAlignment(Qt.AlignmentFlag.AlignCenter)
            s_lay.addWidget(empty_lbl)
        else:
            for inc in incidents:
                card = QFrame()
                card.setFrameShape(QFrame.Shape.StyledPanel)
                c_sev = inc["severity"].upper()
                c_border = C.RED if c_sev == "CRITICAL" else (C.ACC if c_sev == "HIGH" else (C.PRI if c_sev == "MEDIUM" else C.GREEN))
                card.setStyleSheet(f"background: {C.PANEL2}; border: 1px solid {c_border}; border-radius: 4px;")
                c_lay = QVBoxLayout(card)
                c_lay.setContentsMargins(8, 6, 8, 6)
                c_lay.setSpacing(3)

                c_top = QHBoxLayout()
                badge = QLabel(f" {c_sev} ")
                badge.setFont(QFont("Courier New", 7, QFont.Weight.Bold))
                b_bg = "#3a0008" if c_sev == "CRITICAL" else ("#3a1a00" if c_sev == "HIGH" else "#201a00")
                badge.setStyleSheet(f"color: {c_border}; background: {b_bg}; border: 1px solid {c_border}; border-radius: 2px;")
                c_top.addWidget(badge)

                type_lbl = QLabel(f"[{inc['type']}]")
                type_lbl.setFont(QFont("Courier New", 7, QFont.Weight.Bold))
                type_lbl.setStyleSheet(f"color: {C.TEXT_MED}; background: transparent; border: none;")
                c_top.addWidget(type_lbl)

                time_lbl = QLabel(inc["timestamp"])
                time_lbl.setFont(QFont("Courier New", 7))
                time_lbl.setStyleSheet(f"color: {C.TEXT_DIM}; background: transparent; border: none;")
                c_top.addWidget(time_lbl)
                c_top.addStretch()
                c_lay.addLayout(c_top)

                sum_lbl = QLabel(inc["summary"])
                sum_lbl.setWordWrap(True)
                sum_lbl.setFont(QFont("Courier New", 7))
                sum_lbl.setStyleSheet(f"color: {C.WHITE}; background: transparent; border: none;")
        ...
        exp_btn = QPushButton("📥 EXPORT SECURITY REPORT")
        exp_btn.setFixedHeight(28)
        exp_btn.setFont(QFont("Courier New", 8, QFont.Weight.Bold))
        exp_btn.setCursor(Qt.CursorShape.PointingHandCursor)
        exp_btn.setStyleSheet(f"""
            QPushButton {{
                background: #000d1a; color: {C.PRI};
                border: 1px solid {C.PRI_DIM}; border-radius: 3px; padding: 0 10px;
            }}
            QPushButton:hover {{ background: {C.PRI_GHO}; border-color: {C.PRI}; }}
        """)
        exp_btn.clicked.connect(self._export_report)
        b_row.addWidget(exp_btn)
    ...
    def _export_report(self):
        try:
            ...
            cyber_shield({"action": "generate_report"})
            self.hide()
        except Exception as e:
            print(f"[IncidentTimeline] Export error: {e}")


class ClipboardPanel(QWidget):
    """Floating panel shown when text is copied — offers quick Arc actions."""
...
class MainWindow(QMainWindow):
    ...
    _gesture_sig    = pyqtSignal(bool, str)  # (active: bool, gesture_name: str)
    ...
    def __init__(self, face_path: str):
        ...
        self._assistant_name: str = (_cfg.get("assistant_name") or "ARC").strip()
        ...
        self.on_interrupt      = None   # callable: () -> None — stop ARC mid-speech
        self.on_voice_change   = None   # callable: () -> None — rebuild session with new voice
        self.on_audio_device_change = None  # callable: () -> None — reopen audio streams
        self.on_voice_settings_change = None # callable: () -> None — update barge-in / headset / PTT in live engine
        self.on_ptt_state_change = None      # callable: (pressed: bool) -> None
        self._confirm_overlay  = None   # live ConfirmBanner, if one is on screen
        self.get_plugins       = None   # callable: () -> list[dict], set by ArcLive
        self.get_plugin_settings = None # callable: () -> list[dict] settings schemas, set by ArcLive
        self.on_wake_toggle    = None   # callable: (enable: bool) -> str, set by ArcLive
        ...
        self._gesture_sig.connect(self._on_gesture_state_change)
    ...
    # --- Live camera & gesture console controls in HUD area ----------------
    def _refresh_camera_list(self) -> None:
        """Populate camera switcher dropdown with connected devices."""
        if not hasattr(self, "_cam_combo"):
            return
        try:
            from core.device_service import list_connected_cameras, get_active_camera_index
            cams = list_connected_cameras()
            active = get_active_camera_index()
            self._cam_combo.blockSignals(True)
            self._cam_combo.clear()
            for c in cams:
                lbl = f"[{c['index']}] {c['name']}"
                if c["index"] == active:
                    lbl += " [ACTIVE]"
                self._cam_combo.addItem(lbl, c["index"])
                if c["index"] == active:
    ...
    def _on_camera_selected(self, idx: int) -> None:
        if idx < 0 or not hasattr(self, "_cam_combo"):
            return
        cam_idx = self._cam_combo.currentData()
        if cam_idx is not None:
            try:
                from core.device_service import set_active_camera, get_active_camera_name
                set_active_camera(cam_idx)
                if hasattr(self, "_cam_info_lbl"):
                    self._cam_info_lbl.setText(f"CAMERA: {get_active_camera_name()} | 30 FPS")
                self.stop_camera_stream()
                QTimer.singleShot(400, self.start_camera_stream)
            except Exception as e:
                print(f"[Camera] Selection error: {e}")

    def _toggle_camera_console(self) -> None:
        """Toggle Camera Console stack between Animated HUD and Camera View."""
            ...
            if hasattr(self, "_cam_btn"):
                self._cam_btn.setStyleSheet(f"""
                    QPushButton {{
                        ...
                        border: 1px solid {C.PRI}; border-radius: 4px; padding: 3px 8px;
                    }}
                """)

    def _toggle_gesture_control(self) -> None:
        """Toggle MediaPipe hand tracking background thread."""
        try:
    ...
    def _on_ai_vision_scan(self) -> None:
        """Trigger AI scene object detection on the active camera."""
        def _scan_worker():
            try:
                from actions.vision_intelligence import vision_intelligence
                res = vision_intelligence({"action": "detect_objects"})
                self._log_sig.emit(res)
                if hasattr(self, "request_say") and callable(self.request_say):
                    lines = [l for l in res.splitlines() if l.strip()]
                    summary = lines[0] if lines else "Visual scan complete."
                    self.request_say(summary)
            except Exception as e:
                self._log_sig.emit(f"SYS: Vision scan error: {e}")
        threading.Thread(target=_scan_worker, daemon=True, name="ai-vision-scan").start()

    def _on_gesture_state_change(self, active: bool, gesture: str) -> None:
        """Slot — update all gesture pills and HUD telemetry indicators."""
        # 1. Update Header Button
        if hasattr(self, "_hdr_gesture_btn"):
            self._hdr_gesture_btn.setText("🖐 GESTURES: ON" if active else "🖐 GESTURES: OFF")
            col = C.GREEN if active else C.TEXT_DIM
            border = C.GREEN if active else C.BORDER
            self._hdr_gesture_btn.setStyleSheet(f"""
                QPushButton {{
        ...
        # 2. Update Console Button
        if hasattr(self, "_cam_gesture_btn"):
            self._cam_gesture_btn.setText("🖐 GESTURES: ON ✅" if active else "🖐 GESTURES: OFF")
            col = C.GREEN if active else C.TEXT_DIM
            border = C.GREEN if active else C.BORDER
            self._cam_gesture_btn.setStyleSheet(f"""
                QPushButton {{
        ...
        # 3. Update Left Panel Button
        if hasattr(self, "_pill_gesture"):
            col = C.GREEN if active else C.TEXT_DIM
            border = C.GREEN_D if active else C.BORDER_A
            self._pill_gesture.setText("GESTURE\nACTIVE" if active else "GESTURE\nCONTROL")
            self._pill_gesture.setStyleSheet(f"""
                QPushButton {{
        ...
        if hasattr(self, "_gesture_detected_lbl"):
            if gesture and gesture not in ("None", "None / Idle"):
                ...
                self._gesture_detected_lbl.setText("DETECTED: None")
                self._gesture_detected_lbl.setStyleSheet(
                    f"color: {C.TEXT_DIM}; background: {C.PANEL2}; border: 1px solid {C.BORDER}; border-radius: 3px; padding: 2px 8px;"
                )
    ...
    def _on_cam_stream(self, start: bool) -> None:
        if start:
            ...
            self._refresh_camera_list()
            try:
                from core.device_service import get_active_camera_name
                if hasattr(self, "_cam_info_lbl"):
                    self._cam_info_lbl.setText(f"CAMERA: {get_active_camera_name()} | 30 FPS")
            except Exception:
                pass
    ...
    def _cam_loop(self) -> None:
        try:
            ...
            if ctrl.is_running():
                ctrl.on_frame_callback = lambda data: self._cam_frame_sig.emit(data)
                while not self._cam_stop.wait(0.2) and ctrl.is_running():
                    pass
    ...
    def _build_jarvis_icon(out_path: Path) -> bool:
        """
        Render a ARC arc-reactor icon at 4× resolution and downsample
    ...
    def keyPressEvent(self, event):
        if event.key() == Qt.Key.Key_Space and not event.isAutoRepeat():
            from memory.config_manager import get_ptt_enabled
            if get_ptt_enabled():
                focus_w = QApplication.focusWidget()
                if not isinstance(focus_w, (QLineEdit, QTextEdit)):
                    if self.on_ptt_state_change:
                        self.on_ptt_state_change(True)
                    event.accept()
                    return
    ...
    def keyReleaseEvent(self, event):
        if event.key() == Qt.Key.Key_Space and not event.isAutoRepeat():
            ...
            if get_ptt_enabled():
                ...
                if not isinstance(focus_w, (QLineEdit, QTextEdit)):
                    if self.on_ptt_state_change:
                        self.on_ptt_state_change(False)
                    event.accept()
                    return
    ...
    def _update_metrics(self):
        ...
        # VRAM (Dedicated GDDR6)
        vram_pct = snap.get("vram_pct", 0.0)
        vram_used = snap.get("vram_used_mb", 0.0)
        if hasattr(self, "_bar_vram"):
            if vram_used > 0:
                if vram_used >= 1024:
                    self._bar_vram.set_value(vram_pct, f"{vram_used/1024:.1f}G")
                else:
                    self._bar_vram.set_value(vram_pct, f"{vram_used:.0f}M")
            elif vram_pct > 0:
                self._bar_vram.set_value(vram_pct, f"{vram_pct:.0f}%")
            else:
    ...
    def _build_header(self) -> QWidget:
        ...
        # ── Camera Console Trigger Button ──
        self._cam_btn = QPushButton("📷 CAM")
        self._cam_btn.setFont(QFont("Courier New", 8, QFont.Weight.Bold))
        self._cam_btn.setCursor(Qt.CursorShape.PointingHandCursor)
        self._cam_btn.setToolTip("Open Optical Sensor & Gesture Console")
        self._cam_btn.setStyleSheet(f"""
            QPushButton {{
                background: {C.PANEL2}; color: {C.TEXT_MED};
                border: 1px solid {C.BORDER}; border-radius: 4px; padding: 3px 8px;
            ...
            QPushButton:hover {{ color: {C.PRI}; border-color: {C.PRI}; background: {C.PRI_GHO}; }}
        """)
        self._cam_btn.clicked.connect(self._toggle_camera_console)
        lay.addWidget(self._cam_btn)
        lay.addSpacing(6)

        # ── Hand Gesture Pill / Trigger ──
        self._hdr_gesture_btn = QPushButton("🖐 GESTURES: OFF")
        self._hdr_gesture_btn.setFont(QFont("Courier New", 8, QFont.Weight.Bold))
        self._hdr_gesture_btn.setCursor(Qt.CursorShape.PointingHandCursor)
        self._hdr_gesture_btn.setToolTip("Toggle AI Hand Gesture Recognition")
        self._hdr_gesture_btn.setStyleSheet(f"""
            QPushButton {{
                background: {C.PANEL2}; color: {C.TEXT_DIM};
                border: 1px solid {C.BORDER}; border-radius: 4px; padding: 3px 8px;
        ...
        """)
        self._hdr_gesture_btn.clicked.connect(self._toggle_gesture_control)
        lay.addWidget(self._hdr_gesture_btn)
        lay.addStretch()
        ...
        _sub_text = ("Adaptive Real-time Cognitive Agent"
                     if _disp in ("ARC", "A.R.C", "ARC", "J.A.R.V.I.S")
        ...
        top_h = QHBoxLayout(); top_h.setSpacing(6); top_h.addStretch()
        self._pill_threat_counter = QLabel("✓ SECURE")
        self._pill_threat_counter.setFont(QFont("Courier New", 7, QFont.Weight.Bold))
        self._pill_threat_counter.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self._pill_threat_counter.setStyleSheet(
            f"color: {C.GREEN}; border: 1px solid {C.GREEN_D}; background: {C.PANEL2}; border-radius: 3px; padding: 2px 6px;"
        )
        top_h.addWidget(self._pill_threat_counter)

        self._clock_lbl = QLabel("00:00:00")
        self._clock_lbl.setFont(QFont("Courier New", 14, QFont.Weight.Bold))
        self._clock_lbl.setStyleSheet(f"color: {C.PRI}; background: transparent;")
        self._clock_lbl.setAlignment(Qt.AlignmentFlag.AlignRight)
        top_h.addWidget(self._clock_lbl)
        right_col.addLayout(top_h)
    ...
    def _build_left_panel(self) -> QWidget:
        ...
        self._bar_vram = MetricBar("VRAM", "#00FFAA")
        self._bar_tmp = MetricBar("TMP", "#ff6688")

        for bar in [self._bar_cpu, self._bar_mem, self._bar_net,
                    self._bar_gpu, self._bar_vram, self._bar_tmp]:
            lay.addWidget(bar)
        ...
        self._def_panel = QWidget()
        self._def_panel.setStyleSheet(
            f"background: {C.PANEL2}; border: 1px solid {C.BORDER}; border-radius: 4px;"
        )
        def_lay = QVBoxLayout(self._def_panel)
        def_lay.setContentsMargins(6, 5, 6, 5)
        def_lay.setSpacing(2)

        def_hdr = QLabel("🛡 DEFENDER")
        def_hdr.setFont(QFont("Courier New", 7, QFont.Weight.Bold))
        def_hdr.setStyleSheet(f"color: {C.PRI}; background: transparent; border: none;")
        def_lay.addWidget(def_hdr)

        self._def_rt_lbl = QLabel("Real-Time  ON ✓")
        self._def_rt_lbl.setFont(QFont("Courier New", 7))
        self._def_rt_lbl.setStyleSheet(f"color: {C.GREEN}; background: transparent; border: none;")
        def_lay.addWidget(self._def_rt_lbl)

        self._def_sig_lbl = QLabel("Signatures Current")
        self._def_sig_lbl.setFont(QFont("Courier New", 7))
        self._def_sig_lbl.setStyleSheet(f"color: {C.TEXT_MED}; background: transparent; border: none;")
        def_lay.addWidget(self._def_sig_lbl)

        self._def_scan_lbl = QLabel("Scan State Idle")
        self._def_scan_lbl.setFont(QFont("Courier New", 7))
        self._def_scan_lbl.setStyleSheet(f"color: {C.TEXT_MED}; background: transparent; border: none;")
        def_lay.addWidget(self._def_scan_lbl)

        lay.addWidget(self._def_panel)
        lay.addSpacing(4)
        ...
        # CAMERA CONSOLE BUTTON
        self._btn_cam_left = QPushButton("CAMERA\nCONSOLE")
        self._btn_cam_left.setFont(QFont("Courier New", 7, QFont.Weight.Bold))
        self._btn_cam_left.setCursor(Qt.CursorShape.PointingHandCursor)
        self._btn_cam_left.setStyleSheet(f"""
            QPushButton {{
            ...
            QPushButton:hover {{ background: {C.PRI_GHO}; border-color: {C.PRI}; }}
        """)
        self._btn_cam_left.clicked.connect(self._toggle_camera_console)
        lay.addWidget(self._btn_cam_left)

        # GESTURE ACTIVE / TOGGLE BUTTON
        self._pill_gesture = QPushButton("GESTURE\nCONTROL")
        self._pill_gesture.setFont(QFont("Courier New", 7, QFont.Weight.Bold))
        self._pill_gesture.setCursor(Qt.CursorShape.PointingHandCursor)
        self._pill_gesture.setStyleSheet(f"""
            QPushButton {{
            ...
            QPushButton:hover {{ color: {C.GREEN}; border-color: {C.GREEN}; }}
        """)
        self._pill_gesture.clicked.connect(self._toggle_gesture_control)
        lay.addWidget(self._pill_gesture)

        # MEMORY pill showing episodic memory count
        try:
            from memory.memory_manager import retrieve_episodes
            _mem_cnt = len(retrieve_episodes(limit=100))
        except Exception:
            _mem_cnt = 0
        self._pill_memory = QLabel(f"MEMORY\n{_mem_cnt} EPISODES")
        self._pill_memory.setFont(QFont("Courier New", 7, QFont.Weight.Bold))
        self._pill_memory.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self._pill_memory.setStyleSheet(
            f"color: {C.ACC2}; background: {C.PANEL2};"
            f"border: 1px solid {C.BORDER_A}; border-radius: 3px; padding: 4px;"
        )
        lay.addWidget(self._pill_memory)

        # CYBER SHIELD STATUS BUTTON
        self._btn_cyber_shield = QPushButton("CYBER SHIELD\nALL CLEAR")
        self._btn_cyber_shield.setFont(QFont("Courier New", 7, QFont.Weight.Bold))
        self._btn_cyber_shield.setCursor(Qt.CursorShape.PointingHandCursor)
        self._btn_cyber_shield.setStyleSheet(f"""
            QPushButton {{
                color: {C.GREEN}; background: {C.PANEL2};
                border: 1px solid {C.GREEN_D}; border-radius: 3px; padding: 4px;
            }}
            QPushButton:hover {{ background: #002211; border-color: {C.GREEN}; }}
        """)
        self._btn_cyber_shield.clicked.connect(self._open_incident_timeline)
        lay.addWidget(self._btn_cyber_shield)
    ...
    def _build_quick_drawer(self) -> QWidget:
        ...
        incidents_btn = QPushButton("🛡  INCIDENT TIMELINE")
        incidents_btn.setFixedHeight(26)
        incidents_btn.setFont(QFont("Courier New", 7))
        incidents_btn.setCursor(Qt.CursorShape.PointingHandCursor)
        incidents_btn.setStyleSheet(_BTN_STYLE_DIM)
    ...
    def _build_footer(self) -> QWidget:
        ...
        lay.addStretch()
        lay.addWidget(_fl("By Bhavesh Sai", C.PRI_DIM))
        return w
    ...
    def _check_autostart(self) -> bool:
        ...
        try:
            if _OS == "Windows":
                ...
                try:
                    winreg.QueryValueEx(key, "ARC_AI")
                    return True
    ...
    def _toggle_autostart(self):
        ...
        try:
            ...
            if _OS == "Windows":
                ...
                if currently_on:
                    winreg.DeleteValue(reg, "ARC_AI")
                else:
                    pythonw = Path(sys.executable).parent / "pythonw.exe"
                    exe = str(pythonw if pythonw.exists() else sys.executable)
                    winreg.SetValueEx(reg, "ARC_AI", 0, winreg.REG_SZ,
                                      f'"{exe}" "{script}"')
                winreg.CloseKey(reg)
            elif _OS == "Darwin":
    ...
    def _wake_state(self) -> dict:
        ...
        if self.wake_get_state:
            try:
                ...
                pass
        # Before ArcLive has wired its callback (drawer built at startup).
    ...
    def _toggle_wake_word(self):
        ...
        if not st["ready"]:
            ...
            return
        # Already downloaded → just flip enabled/disabled through ArcLive.
    ...
    def _open_customize(self):
        ...
        cw = self.centralWidget()
        ov = CustomizeOverlay(
            cfg.get("assistant_name", "ARC") or "ARC",
            cfg.get("user_name", ""),
    ...
    def _apply_name_update(self, name: str, user_name: str, ui_color: str = "",
        ...
        self._assistant_name = name.strip() or "ARC"
        display = self._assistant_name.upper()
        self.setWindowTitle(f"{display} — {APP_VERSION}")
        self._title_lbl.setText(display)
        if display in ("ARC", "A.R.C"):
            self._sub_lbl.setText("Adaptive Real-Time Cognitive Agent")
        else:
        ...
        try:
            from memory.config_manager import _write_config
    ...
    def _open_audio_devices(self):
        try:
            ...
            self._log.append_log(f"ERR: Failed to open audio settings: {e}")
    ...
    def _open_incident_timeline(self):
        ov = IncidentTimelineOverlay(parent=self.centralWidget())
        self._centre_overlay(ov)
    ...
    def _update_cyber_shield_btn(self, severity: str, count: int = 0):
        ...
        if sev == "CRITICAL":
            self._btn_cyber_shield.setText(f"CYBER SHIELD\nCRITICAL ({count})")
            self._btn_cyber_shield.setStyleSheet(f"""
                QPushButton {{
                    color: {C.RED}; background: #330006;
            ...
            self._btn_cyber_shield.setStyleSheet(f"""
                QPushButton {{
                    color: {C.ACC}; background: #2a1000;
                    border: 1px solid {C.ACC}; border-radius: 3px; padding: 4px;
                }}
            """)
            ...
            self._btn_cyber_shield.setStyleSheet(f"""
                QPushButton {{
                    color: #ffcc00; background: #262000;
                    border: 1px solid #ffcc00; border-radius: 3px; padding: 4px;
                }}
            """)
        elif sev == "OFFLINE":
            self._btn_cyber_shield.setText("CYBER SHIELD\nOFFLINE")
            self._btn_cyber_shield.setStyleSheet(f"""
                QPushButton {{
                    color: #777777; background: {C.PANEL2};
    ...
    def _update_threat_pill(self, count: int = 0):
        if not hasattr(self, "_pill_threat_counter"):
            return
        if count > 0:
            self._pill_threat_counter.setText(f"⚠ {count} THREAT{'S' if count > 1 else ''}")
            self._pill_threat_counter.setStyleSheet(
                f"color: {C.RED}; border: 1px solid {C.RED}; background: #2a0005; border-radius: 3px; padding: 2px 6px;"
            )
        else:
            self._pill_threat_counter.setText("✓ SECURE")
            self._pill_threat_counter.setStyleSheet(
                f"color: {C.GREEN}; border: 1px solid {C.GREEN_D}; background: {C.PANEL2}; border-radius: 3px; padding: 2px 6px;"
            )

    def _set_threat_state(self, severity: str, count: int = 0):
        self._update_cyber_shield_btn(severity, count)
        self._update_threat_pill(count)
        if hasattr(self, "hud"):
            self.hud.set_threat_level(severity)
    ...
    def _on_setup_done(self, key: str, os_name: str):
        from memory.config_manager import _write_config
        d = _read_full_config()
        d.update({"gemini_api_key": key, "os_system": os_name})
        _write_config(d)
        self._ready = True
        if self._overlay:
            self._overlay.hide()
            self._overlay = None
        self._apply_state("LISTENING")
        self._assistant_name = _read_full_config().get("assistant_name", "ARC") or "ARC"
...
class ArcUI:
    ...
    def on_voice_settings_change(self):
        return self._win.on_voice_settings_change

    @on_voice_settings_change.setter
    def on_voice_settings_change(self, cb):
        self._win.on_voice_settings_change = cb

    @property
    def on_ptt_state_change(self):
        return self._win.on_ptt_state_change

    @on_ptt_state_change.setter
    def on_ptt_state_change(self, cb):
        self._win.on_ptt_state_change = cb
    ...
    def open_remote(self) -> None:
        """Thread-safe: open remote control QR code overlay."""
        try:
            from PyQt6.QtCore import QTimer
            QTimer.singleShot(0, self._win._open_remote)
        except Exception:
            try:
    ...
    def set_gesture_active(self, active: bool, gesture: str = "") -> None:
        """Thread-safe: update gesture recognition state across all HUD and console widgets."""
        try:
            if hasattr(self._win, "_gesture_sig"):
                self._win._gesture_sig.emit(bool(active), str(gesture))
        except Exception:
            pass

    def update_memory_count(self, count: int) -> None:
        """Thread-safe: update the MEMORY pill episode count."""
        try:
            if hasattr(self._win, "_pill_memory"):
                QTimer.singleShot(0, lambda: self._win._pill_memory.setText(f"MEMORY\n{count} EPISODES"))
        except Exception:
            pass

    def set_threat_state(self, severity: str, count: int = 0) -> None:
        """Thread-safe: update Cyber Shield state across sphere, status button, and threat pill."""
        try:
            QTimer.singleShot(0, lambda: self._win._set_threat_state(severity, count))
        except Exception:
            pass

    def update_defender_hud(self, real_time: bool = True, sig_status: str = "Current", scan_state: str = "Idle") -> None:
        """Thread-safe: update Defender status panel on the HUD."""
        def _apply():
            try:
                if hasattr(self._win, "_def_rt_lbl"):
                    col = C.GREEN if real_time else C.RED
                    self._win._def_rt_lbl.setText(f"Real-Time  {'ON ✓' if real_time else 'OFF ✗'}")
                    self._win._def_rt_lbl.setStyleSheet(f"color: {col}; background: transparent; border: none;")
                if hasattr(self._win, "_def_sig_lbl"):
                    self._win._def_sig_lbl.setText(f"Signatures {sig_status}")
                if hasattr(self._win, "_def_scan_lbl"):
                    self._win._def_scan_lbl.setText(f"Scan State {scan_state}")
            except Exception:
                pass
        try:
