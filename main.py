...
_os.environ.setdefault("QSG_RHI_BACKEND", "d3d11")
_os.environ.setdefault("QT_QUICK_BACKEND", "rhi")
_os.environ.setdefault("QT_D3D_ADAPTER_INDEX", "0")
_os.environ.setdefault("SHIM_MCCOMPAT", "0x800000001")
...
from ui import ArcUI, ArcUI
from memory.memory_manager import (
    load_memory, update_memory, format_memory_for_prompt,
    save_session_summary, pop_last_session,
    search_memory, set_trim_notifier,
    detect_and_ingest_feedback, format_semantic_context_for_prompt,
    retrieve_semantic_memories,
    save_episodic_memory, retrieve_episodes,
    load_pending_tasks, save_pending_task, get_active_pending_tasks,
)
...
# (screen_process, close_camera, save_memory, manage_monitor, shutdown_arc,
# system_status).
from actions.screen_processor  import _capture_camera, _capture_screen, start_screen_ocr_buffer, get_recent_screen_context
...
from core.greeting             import build_startup_greeting_prompt
from core.logger               import get_logger, sanitize_message
...
# Voice activity needed to interrupt a response.  Requiring several consecutive
# microphone blocks avoids stopping for a click or a short burst of room noise.
# A headset gives the best barge-in experience because speaker echo can otherwise
# resemble the user's voice to any microphone-only detector.
_BARGE_IN_LEVEL  = 0.44
_BARGE_IN_BLOCKS = 7
...
TOOL_DECLARATIONS = [
    ...
    # core.action_loader (see ArcLive.__init__).
    {
        "name": "system_status",
        "description": (
    ...
    },
    {
        ...
        "description": (
            ...
            "ARC checks these topics once a day and alerts the user when there is a new development. "
    ...
    },
    {
        ...
        "description": (
            ...
            "close the assistant, say goodbye, or stop Arc. "
    ...
    },
    {
        ...
        "description": (
            "Backward compatibility alias for shutdown_arc. Shuts down the assistant completely."
        ),
        "parameters": {
            "type": "OBJECT",
            "properties": {},
...
class ArcLive:
    def __init__(self, ui: ArcUI):
        self.ui             = ui
        self._asst_name     = "ARC"       # updated each session from config
        self.session              = None
        ...
        self._barge_in_blocks      = 0       # consecutive voiced mic blocks while speaking
        self._last_barge_in        = 0.0     # debounce automatic interruptions
        self._barge_in_sens        = get_barge_in_sensitivity()
        self._headset_mode         = get_headset_mode()
        self._ptt_enabled          = get_ptt_enabled()
        ...
        self.ui.on_voice_settings_change = self._on_voice_settings_change
        self.ui.on_ptt_state_change = self._set_ptt_state
        ...
        self._session_tools: set[str] = set()      # tools executed in this session
        self._turn_count: int = 0                  # turn counter for rolling summarization
        self._current_emotion: dict | None = None  # detected user voice emotion
        self._user_voice_chunks: list[bytes] = []  # voice frames for diarization & emotion
        self._is_offline: bool = False             # offline fallback flag

        try:
            start_screen_ocr_buffer(30.0)
        except Exception:
            pass
        ...
        try:
            ...
            ArcSelfAwareness.get_instance().set_registries(self._action_registry, self._plugin_registry)
        except Exception:
            pass
        ...
        # ── ARC Cyber Shield Background Daemon ────────────────────────────────
        try:
            from plugins.threat_monitor import start_monitor, set_callbacks
            set_callbacks(
                speak_cb=self.plugin_say,
                log_cb=self.ui.write_log,
                hud_cb=self.ui.set_threat_state,
            )
            start_monitor()
        except Exception as e:
            print(f"[CyberShield] Background daemon initialization: {e}")

        # ── Windows Defender Event Watcher Daemon ────────────────────────────
        try:
            from plugins.defender_monitor import start_defender_monitor, set_defender_callbacks
            set_defender_callbacks(
                speak_cb=self.plugin_say,
                log_cb=self.ui.write_log,
                hud_cb=self.ui.set_threat_state,
            )
            start_defender_monitor()
        except Exception as e:
            print(f"[DefenderMonitor] Initialization failed: {e}")
    ...
    def _ensure_wake_detector(self) -> bool:
        ...
        if self._wake_detector is None:
            self._wake_detector = WakeWordDetector(
                ...
                is_playback_active=lambda: bool(getattr(self, "_is_speaking", False)),
    ...
    def _on_wake_detected(self) -> None:
        """Called from the detector thread when 'Hey Arc' or 'Hey Arc' is heard."""
    ...
    def sleep(self, reason: str = "timeout") -> None:
        ...
        self.ui.write_log(f"SYS: Sleeping — {reason}. Say 'Hey Arc' to wake me.")
    ...
    def plugin_say(self, instruction: str) -> None:
        """
        Thread-safe speech channel for plugins: lets a plugin ask ARC to
    ...
    def _on_voice_settings_change(self):
        """Voice settings (barge-in sensitivity, headset mode, PTT) updated."""
        self._barge_in_sens = get_barge_in_sensitivity()
        self._headset_mode  = get_headset_mode()
        self._ptt_enabled   = get_ptt_enabled()
        self._logger.system(
            f"Voice settings updated: sensitivity={self._barge_in_sens}, "
            f"headset={self._headset_mode}, ptt={self._ptt_enabled}"
        )
        sens_str = (self._barge_in_sens or "medium").capitalize()
        hs_str = "ON" if self._headset_mode else "OFF"
        ptt_str = "ON" if self._ptt_enabled else "OFF"
        self.ui.write_log(
            f"SYS: Voice settings updated (Sens: {sens_str}, Headset: {hs_str}, PTT: {ptt_str})."
        )

    def _set_ptt_state(self, pressed: bool):
        """Push-to-talk button state changed."""
        self._ptt_pressed = pressed
        if pressed:
            if self._is_speaking and self._loop and self._loop.is_running():
                self._loop.call_soon_threadsafe(self.interrupt)
            self.ui.write_log("SYS: Push-to-Talk active (listening).")
        else:
            self.ui.write_log("SYS: Push-to-Talk released.")

    def _get_barge_in_params(self) -> tuple[float, int]:
        sens = (self._barge_in_sens or "medium").lower()
        if sens == "high":
            thresh = 0.32
            blocks = 5
        elif sens == "low":
        ...
        # When headset mode is off (using external laptop speakers),
        # raise threshold and blocks significantly to prevent speaker feedback from interrupting Arc
        if not self._headset_mode:
            thresh += 0.12
            blocks += 2

        return thresh, blocks
    ...
    def _on_text_command(self, text: str):
        if not self._loop:
            return
        if not self.session or getattr(self, "_is_offline", False):
            # Offline fallback routing (Task 14)
            asyncio.run_coroutine_threadsafe(
                self.route_offline_query(text),
                self._loop
            )
            return
        # Respect wake-word sleep: a typed command must not be answered while
        # asleep either (the sleep gate is not just for the mic). Wake first with
        # "Hey Arc" or the WAKE NOW button.
        if self._wake_enabled and not self._awake:
            self.ui.write_log("SYS: I'm asleep — say 'Hey Arc' or tap WAKE NOW first.")
            return
        asyncio.run_coroutine_threadsafe(
    ...
    def _broadcast_state(self, state: str) -> None:
        if self._dashboard and self._loop and self._loop.is_running():
            try:
                asyncio.run_coroutine_threadsafe(
                    self._dashboard.broadcast({"type": "state", "state": state.lower()}),
                    self._loop,
                )
            except Exception:
                pass
    ...
    def interrupt(self) -> None:
        """Stop ARC mid-speech: drain queued audio and open mic immediately."""
        ...
        if q:
            ...
            while True:
                try:
                    ...
                    break
            if drained:
                print(f"[ARC] ✋ Interrupted — {drained} audio chunks discarded")
        self.set_speaking(False)
    ...
    def speak(self, text: str):
        if not self._loop:
            return
        if self.session and not getattr(self, "_is_offline", False):
            asyncio.run_coroutine_threadsafe(
                self.session.send_client_content(
                    turns={"role": "user", "parts": [{"text": text}]},
                    turn_complete=True
            ...
            def _bg_local_tts():
                try:
                    ...
                    print(f"[ARC] Local TTS error: {_e}")
            threading.Thread(target=_bg_local_tts, daemon=True).start()
    ...
    def _build_config(self) -> types.LiveConnectConfig:
        ...
        try:
            from memory.config_manager import load_api_keys
            _cfg = load_api_keys()
            self._asst_name = (_cfg.get("assistant_name") or "ARC").strip()
        ...
        try:
            from core.team_memory import get_team_context_str
            team_str = get_team_context_str()
            if team_str:
                parts.append("\n" + team_str + "\n")
        except Exception:
            pass
        ...
        try:
            from core.self_awareness import ArcSelfAwareness
            caps_str = ArcSelfAwareness.get_instance().format_capabilities_for_prompt()
            if caps_str:
                parts.append("\n" + caps_str + "\n")
        except Exception:
            pass
        ...
        cfg = dict(
            ...
            # window — ARC can stay in one conversation for hours
        ...
        if self._enhanced_live:
            # Proactive audio: ARC stays silent when speech isn't addressed
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        self._session_tools.add(name)

        print(f"[ARC] 🔧 {name}  {args}")
        ...
        try:
            ...
            elif name in ("shutdown_arc", "shutdown_jarvis"):
                ...
                async def _do_shutdown():
                    ...
                    if self.session:
                        try:
                            ...
                            pass
                    try:
                        from plugins.threat_monitor import stop_monitor
                        stop_monitor()
                    except Exception:
                        pass
                    try:
                        from plugins.defender_monitor import stop_defender_monitor
                        stop_defender_monitor()
                    except Exception:
                        pass
        ...
        print(f"[ARC] 📤 {name} → {str(result)[:80]}")
        return types.FunctionResponse(
            id=fc.id, name=name,
    ...
    async def _send_realtime(self):
        while True:
            ...
            # Real-time latency optimization: if out_queue backed up (>12 chunks ≈ 750ms),
            # drain stale audio frames so we always stream current speech with near-zero latency
            while self.out_queue.qsize() > 12:
                try:
    ...
    async def _listen_audio(self):
        print("[ARC] 🎤 Mic started")
        loop = asyncio.get_event_loop()

        def callback(indata, frames, time_info, status):
            # ── Wake-word gate ───────────────────────────────────────────────
            # While asleep, the mic audio NEVER goes to Gemini (nothing is
            # streamed, so ARC can't respond to speech not addressed to it and
            ...
            if self._ptt_enabled and not self._ptt_pressed:
                # Push-to-talk mode active and spacebar not pressed
                return

            with self._speaking_lock:
                arc_speaking = self._is_speaking
            if not self.ui.muted and not self._phone_active:
                data = indata.tobytes()

                # Barge-in: dynamic threshold and required blocks
                if arc_speaking:
                    now = time.monotonic()
                    # Grace period: ignore initial 0.5s of playback to prevent acoustic echo self-interruption
                    if now - getattr(self, "_speaking_start_time", 0.0) < 0.50:
                        self._barge_in_blocks = 0
                        return

                    thresh, req_blocks = self._get_barge_in_params()
                    if _pcm_level(indata) >= thresh:
                        self._barge_in_blocks += 1
                    else:
                        self._barge_in_blocks = 0

                    if (self._barge_in_blocks >= req_blocks
                ...
                self._barge_in_blocks = 0
                if not arc_speaking and _pcm_level(indata) > 0.025:
                    self._user_voice_chunks.append(data)
                    if len(self._user_voice_chunks) > 35:
                        self._user_voice_chunks.pop(0)
        ...
        try:
            def _open_mic(dev):
                try:
                    ...
                    def stereo_cb(indata, frames, time_info, status):
                        mono = indata[:, 0:1]
                        callback(mono, frames, time_info, status)
            ...
            if _mic_dev is not None:
                print(f"[ARC] 🎤 Input device: {_mic_name}")
            ...
            try:
                ...
                if _mic_dev is not None:
                    print(f"[ARC] ⚠️  Mic '{_mic_name}' failed: {_e} — using default")
                    self.ui.write_log(
                    ...
                    try:
                        ...
                        print(f"[ARC] ⚠️  Default microphone also failed: {_e2}")
                else:
                    print(f"[ARC] ⚠️  Default microphone failed: {_e}")

            if _mic_stream is None:
                self.ui.write_log("SYS: Warning: No microphone available. Voice input disabled.")
                while True:
                    await asyncio.sleep(1.0)

            try:
                with _mic_stream:
                    print("[ARC] 🎤 Mic stream open")
                    while True:
                        await asyncio.sleep(0.1)
            except Exception as mic_err:
                print(f"[ARC] ⚠️ Microphone stream error: {mic_err}")
                self.ui.write_log("SYS: Microphone disconnected — attempting recovery...")
                self.request_reconnect(keep_context=True, reason="microphone recovery")
                while True:
                    await asyncio.sleep(1.0)
        except Exception as e:
            print(f"[ARC] ❌ Mic fatal: {e}")

    async def _receive_audio(self):
        print("[ARC] 👂 Recv started")
        out_buf, in_buf = [], []
                    ...
                    if _sru is not None:
                        if getattr(_sru, "resumable", False) and getattr(_sru, "new_handle", None):
                            if self._resume_handle is None:
                                print("[ARC] 🔗 Session resumption armed")
                    ...
                    if response.server_content:
                        ...
                        if sc.turn_complete:
                            ...
                            if full_in:
                                ...
                                # Persistent Personalized Learning: ingest feedback & corrections
                                try:
                                    _prev = self._session_log[-2] if len(self._session_log) >= 2 else ""
                                    _learned = detect_and_ingest_feedback(full_in, previous_asst_turn=_prev)
                                    if _learned:
                                        _msg = f"🧠 Learned ({_learned['type']}): {_learned['lesson']}"
                                        self.ui.write_log(_msg)
                                        if self._dashboard:
                                            asyncio.create_task(self._dashboard.broadcast({
                                                "type": "log", "speaker": "arc",
                                                "text": _msg,
                                                "ts": datetime.now().isoformat(),
                                            }))
                                except Exception as _e:
                                    print(f"[Memory] ⚠️ Feedback ingestion error: {_e}")
                            in_buf = []
                            ...
                            # Emotion detection & speaker diarization capture (async non-blocking)
                            if self._user_voice_chunks:
                                voice_bytes = b"".join(self._user_voice_chunks[-25:])
                                confirm_gate.set_last_speaker_audio(voice_bytes)
                                self._user_voice_chunks.clear()

                            # Task 8: Self-critique loop
                            if full_out and len(full_out) > 35:
                                asyncio.create_task(self._run_self_critique(full_in, full_out))

                            # Task 13: 20-turn conversation summarization checkpoint
                            self._turn_count += 1
                            if self._turn_count % 20 == 0:
                                asyncio.create_task(self._summarize_conversation_history())
                            ...
                            if self._pending_vision and self.session:
                                ...
                                if self._vision_cam_active:
                                    # Camera: keep busy until ARC finishes speaking the answer
    ...
    async def _play_audio(self):
        ...
        def _open_spk(dev):
            ...
            try:
                ...
                # Try 2 channels (stereo) — required by Bluetooth A2DP headsets on Windows
                try:
                    st = sd.RawOutputStream(
                        samplerate=RECEIVE_SAMPLE_RATE,
                        channels=2,
        ...
        stream_channels = 1
        try:
            stream, stream_channels = _open_spk(_spk_dev)
        except Exception as _e:
            if _spk_dev is not None:
                print(f"[ARC] ⚠️  Output device '{_spk_name}' failed: {_e} — using default")
                self.ui.write_log(f"SYS: Speaker '{_spk_name}' unavailable — using system default.")
                try:
                    stream, stream_channels = _open_spk(None)
                except Exception as _e2:
                    print(f"[ARC] ⚠️  Default speaker also failed: {_e2}")
            else:
                print(f"[ARC] ⚠️  Default speaker failed: {_e}")

        if stream is None:
            self.ui.write_log("SYS: Warning: No speaker available. Voice playback disabled.")
        ...
        try:
            while True:
                ...
                raw_pcm = np.frombuffer(bytes(batch), dtype=np.int16)
                # Apply 2.2x digital voice boost (~ +7 dB) with clean limiter to ensure loud, rich, crystal-clear voice
                boosted_pcm = np.clip(raw_pcm.astype(np.float32) * 2.2, -32767.0, 32767.0).astype(np.int16)

                try:
                    self.ui.set_audio_level(_pcm_level(boosted_pcm))
                except Exception:
                    pass

                if stream is not None:
                    # If stream is 2-channel stereo, duplicate mono samples
                    if stream_channels == 2:
                        payload = np.repeat(boosted_pcm, 2).tobytes()
                    else:
                        payload = boosted_pcm.tobytes()
                    ...
                    try:
                        ...
                        # Bluetooth device turned off mid-playback!
                        print(f"[ARC] ⚠️ Playback error ({type(play_err).__name__}): {play_err}")
                        self.ui.write_log("SYS: Audio device disconnected — switching to default speaker.")
                        try:
                        ...
                        try:
                            stream, stream_channels = _open_spk(None)
                            if stream is not None:
                                fallback_payload = np.repeat(boosted_pcm, 2).tobytes() if stream_channels == 2 else boosted_pcm.tobytes()
                                await asyncio.to_thread(stream.write, fallback_payload)
                        except Exception as fallback_err:
                            print(f"[ARC] ⚠️ Fallback to default speaker failed: {fallback_err}")
        except Exception as e:
            print(f"[ARC] ❌ Play fatal: {e}")
        finally:
            self.set_speaking(False)
            if stream is not None:
                try:
    ...
    async def _send_startup_briefing(self) -> None:
        """
        Give a reliable, conversational startup greeting.  This intentionally
        uses only local information: time, stored context, and a local system
        snapshot.  News is available when asked, but a failed network fetch must
        never become the first thing the user hears after launching the app.
        """
        memory   = load_memory()
        ...
        lang = _val("language")
        name = _val("name") or get_user_name() or "Sir"
        ...
        try:
            system = await asyncio.to_thread(get_system_status)
        except Exception as e:
            print(f"[Briefing] System snapshot unavailable: {e}")
            system = None

        last = await asyncio.to_thread(pop_last_session)
        active_tasks = await asyncio.to_thread(get_active_pending_tasks)
        greeting = build_startup_greeting_prompt(
            time_str=datetime.now().strftime("%H:%M"),
    ...
    async def _save_session_summary(self) -> None:
        ...
        try:
            ...
            if summary:
                ...
                tools_used = list(self._session_tools)
                self._session_tools.clear()
                try:
                    save_episodic_memory(summary=summary, tools_used=tools_used)
                except Exception as _ee:
                    print(f"[Memory] Episodic save error: {_ee}")
        except Exception as e:
            print(f"[Memory] ⚠️ Session summary failed: {e}")

    # ── Self-critique & Accuracy Loop (Task 8) ──────────────────────────────────

    async def _run_self_critique(self, user_text: str, asst_text: str) -> None:
        """Asynchronously verify factual and code correctness using gemini-2.5-flash."""
        if not asst_text or len(asst_text) < 40:
            return

        prompt = (
            "You are a strict factual and code verification engine for the AI assistant ARC.\n"
            "Evaluate the assistant's response for factual errors, invalid citations, or broken code.\n\n"
            f"User query: {user_text}\n"
            f"Assistant response: {asst_text}\n\n"
            "Respond in JSON format with keys:\n"
            "- score: integer from 1 to 10\n"
            "- critique: brief critique if score < 7, else empty string\n"
            "- correction: concise corrected fact or code fix if score < 7, else empty string"
        )
        try:
            from google import genai as _genai
            client = _genai.Client(api_key=_get_api_key())
            ...
            critique_data = json.loads(raw)
            score = int(critique_data.get("score", 10))
            if score < 7:
                critique = critique_data.get("critique", "")
                correction = critique_data.get("correction", "")
                warn_msg = f"[Critique Warning] Score {score}/10: {critique}"
                print(f"[ARC] {warn_msg}")
                self.ui.write_log(f"SYS: {warn_msg}")
                if correction:
                    try:
                        from memory.memory_manager import add_semantic_memory
                        add_semantic_memory(
                            lesson=f"Correction: {correction}",
                            concept="self_critique_correction",
                            mem_type="critique",
                            source="self_critique",
                            priority=9,
                            context=f"Corrected response to: {user_text[:60]}",
                        )
                    except Exception:
                        pass
        except Exception:
    ...
    # ── Conversation 20-Turn Summarizer (Task 13) ──────────────────────────────

    async def _summarize_conversation_history(self) -> None:
        """Compress older turns into a dense 3-sentence summary every 20 turns."""
        if len(self._session_log) < 15:
            return
        older_convo = "\n".join(self._session_log[:-6])
        recent_turns = self._session_log[-6:]
        prompt = (
            "Summarize the following conversation history into exactly 3 clear, highly-dense sentences, "
            "preserving key context, user requests, and task progress:\n\n" + older_convo
        )
        try:
            from google import genai as _genai
            client = _genai.Client(api_key=_get_api_key())
            ...
            summary = (resp.text or "").strip()
            if summary:
                self._session_log = [f"[ROLLING SUMMARY]: {summary}"] + recent_turns
                print("[ARC] [Memory] Compressed conversation history into 3-sentence summary.")
                self.ui.write_log("SYS: Conversation history compressed (20-turn checkpoint).")
        except Exception as e:
            print(f"[ARC] Conversation summarization notice: {e}")

    # ── Offline Fallback Routing (Task 14) ──────────────────────────────────────

    async def route_offline_query(self, user_query: str) -> str:
        """Route user query to local LLM when Gemini Live WebSocket is offline."""
        self.ui.write_log(f"SYS: [Offline Fallback] Routing '{user_query[:40]}' to local LLM...")
        try:
            from core.llm_client import call_llm
            system_prompt = (
                f"You are {self._asst_name}, an offline AI assistant. "
                "Cloud connection is temporarily down. Respond clearly and concisely."
            )
            messages = [
                {"role": "system", "content": system_prompt},
                {"role": "user", "content": user_query},
            ]
            resp = await asyncio.to_thread(call_llm, messages, timeout=25)
            answer = resp.get("content", "I am running in offline mode.")
            self.ui.write_log(f"{self._asst_name} (Offline): {answer}")
            return answer
        except Exception as e:
            err = f"Offline fallback unavailable: {e}"
    ...
    async def _run_background_monitor(self) -> None:
        ...
        while True:
            if self.session and self._awake:
                # Don't interrupt if user spoke recently or ARC is mid-sentence
                with self._speaking_lock:
                    speaking = self._is_speaking
    ...
    async def _process_dashboard_commands(self) -> None:
        while True:
            try:
                ...
                bw_state = "HIGH"
                try:
                    from core.bandwidth_monitor import get_bandwidth_state, CRITICAL
                    bw_state = get_bandwidth_state()
                except Exception:
                    pass

                # If connection is healthy, wait up to 1.5s for cloud session
                if bw_state != "CRITICAL":
                    for _ in range(15):
                        if self.session and not getattr(self, "_is_offline", False):
                            break
                        await asyncio.sleep(0.1)

                if bw_state != "CRITICAL" and self.session and not getattr(self, "_is_offline", False):
                    # Ingest any explicit feedback from typed remote commands
                    try:
                        _learned = detect_and_ingest_feedback(text)
                        if _learned:
                            _msg = f"🧠 Learned ({_learned['type']}): {_learned['lesson']}"
                            self.ui.write_log(_msg)
                            if self._dashboard:
                        ...
                        print(f"[Memory] ⚠️ Remote feedback error: {_e}")

                    ctx = format_semantic_context_for_prompt(text)
                    prompt_payload = f"{ctx}\n[USER COMMAND]: {text}" if ctx else text
                    await self.session.send_client_content(
                        turns={"role": "user", "parts": [{"text": prompt_payload}]},
                        turn_complete=True,
                    )
                    self.ui.write_log(f"[Web]: {text}")
                else:
                    # Slow internet / offline fallback — execute locally
                    self.ui.write_log(f"[Web]: {text}")
                    clean = text.strip().lower()
                    handled = False
                    reply = ""

                    if any(k in clean for k in ("mute", "unmute")):
                        self.ui.toggle_mute()
                        reply = "Microphone muted." if self.ui.muted else "Microphone unmuted."
                        handled = True
                    elif "lock" in clean and any(k in clean for k in ("pc", "screen", "workstation", "computer")):
                        try:
                            import ctypes
                            ctypes.windll.user32.LockWorkStation()
                            reply = "Workstation locked successfully."
                            handled = True
                        except Exception as _e:
                            reply = f"Could not lock screen: {_e}"
                            handled = True
                    elif "screenshot" in clean and self._action_registry.has("screen_capture"):
                        reply = self._action_registry.run("screen_capture", {"action": "capture"})
                        handled = True
                    elif any(k in clean for k in ("system status", "battery", "cpu", "ram", "memory")) and self._action_registry.has("system_monitor"):
                        reply = self._action_registry.run("system_monitor", {})
                        handled = True

                    if not handled:
                        reply = await self.route_offline_query(text)
        ...
        while True:
            try:
                ...
                async with (
                    ...
                    # until the user says "Hey Arc" or taps wake in the UI.
                    if self._wake_enabled:
                        self._ensure_wake_detector()
                        self._awake = False
                        self.ui.set_state("SLEEPING")
                        self.ui.write_log("SYS: ARC online — sleeping. Say 'Hey Arc' to wake me.")
                    else:
                        self._awake = True
                        self.ui.set_state("LISTENING")
                        self.ui.write_log("SYS: ARC online.")
                ...
                if _is_reconnect_signal(e):
                    print("[ARC] Voluntary reconnect requested.")
                ...
                if _resumed_with and (
                    ...
                    print("[ARC] 🔗 Resumption handle rejected — starting a fresh session")
                ...
                err_str = str(e)
                print(f"[ARC] Error ({type(e).__name__}): {e}")
                traceback.print_exc()
                ...
                if "API key not valid" in err_str or "1007" in err_str:
                    ...
                    print("[ARC] New API key saved — reconnecting...")
                    _conn_backoff = 3
                    continue
                ...
                if is_net_err:
                    ...
                    self.ui.write_log(
                        f"NET: Connection failed — retrying in {_conn_backoff}s. Local LLM fallback engaged."
                    )
                else:
            ...
            print(f"[ARC] Reconnecting in {delay}s...")
            await asyncio.sleep(delay)

def main():
    try:
        from core.gpu_accelerator import init_gpu_acceleration, GPUAccelerator
        gpu_stat = init_gpu_acceleration()
        GPUAccelerator.configure_qt_gpu_acceleration()
        if gpu_stat.get("active"):
            print(f"[ARC] 🚀 Discrete GPU VRAM Acceleration Active: {gpu_stat.get('gpu_name')} ({gpu_stat.get('vram_total_mb')} MB GDDR6 VRAM)")
    except Exception as _gpu_e:
        print(f"[ARC] GPU hardware accelerator notice: {_gpu_e}")
...
if __name__ == "__main__":
    if "--health-check" in sys.argv or "--self-check" in sys.argv:
        from core.self_check import run_self_check, format_self_check_report
        res = run_self_check()
        print(format_self_check_report(res))
