"""
dashboard/server.py — ARC Local HTTP Dashboard
...
def _make_uploads_dir() -> Path:
    ...
    for candidate in [
        ...
        Path.home() / "Documents" / "ARC Uploads",
        BASE_DIR / "uploads",
    ]:
        try:
...
def _ensure_network_access(port: int) -> None:
    ...
    if sys.platform == "win32":
        ...
        port_rule = f"ARC Dashboard Port {port}"
        prog_rule  = "ARC Dashboard Python"
        ...
        try:
            ...
            if int(ret) > 32:
                ...
                print("[Dashboard] Phone connections may fail until ARC is run as Administrator.")
...
def _ensure_crypto_js() -> None:
    ...
    def _bg_download():
        try:
            ...
            print(f"[Dashboard] CryptoJS download failed (running in offline/slow mode): {e}")
...
def _local_ip() -> str:
    ...
    # Method 1: fast UDP connect trick to local gateway or LAN broadcast (no packet sent, instantaneous)
    for probe in ("192.168.1.1", "192.168.0.1", "10.0.0.1", "1.1.1.1", "8.8.8.8"):
        try:
    ...
    # Method 2: hostname resolution (works offline on local network)
    try:
...
_dashboard_extension_routes: list = []


def get_dashboard_server() -> "DashboardServer | None":
    return _dashboard_instance


def register_dashboard_route(fn) -> None:
    """Allow plugins to register routes and websockets on the main FastAPI server."""
    if fn not in _dashboard_extension_routes:
        _dashboard_extension_routes.append(fn)
    if _dashboard_instance and hasattr(_dashboard_instance, "app") and _dashboard_instance.app:
        try:
            fn(_dashboard_instance.app, _dashboard_instance)
        except Exception as e:
            print(f"[Dashboard] Extension route registration error: {e}")
...
class DashboardServer:
    ...
    def __init__(self):
        ...
        self._active_key_sessions: dict[str, dict] = {}  # key -> {token, device_token, session_key, expires}
    ...
    def new_key(self, expiry_secs: int = 600) -> str:
        ...
        self._active_key_sessions = {
            k: v for k, v in self._active_key_sessions.items() if v.get("expires", 0) > now
        }
    ...
    async def broadcast(self, msg: dict, priority: str = "AUTO") -> None:
        msg_type = msg.get("type", "")
        if priority == "AUTO":
            if msg_type in ("command", "command_result", "sys", "alert", "auth", "defender_alert", "screen_mirror_state", "health_alert", "error", "bandwidth_state"):
                priority = "CRITICAL"
            elif msg_type in ("transcript", "ai_response", "notes", "file_received", "action"):
                priority = "HIGH"
            else:
                priority = "LOW"

        try:
            from core.bandwidth_monitor import get_bandwidth_state, CRITICAL, LOW
            bw_state = get_bandwidth_state()
            if bw_state == CRITICAL and priority == "LOW":
                return
            if bw_state == LOW and priority == "LOW":
                now = time.monotonic()
                if getattr(self, "_last_low_broadcast", 0) > now - 2.0:
                    return
                self._last_low_broadcast = now
        except Exception:
            pass
    ...
    def _build_app(self) -> "FastAPI":
        ...
        def _auth(req: Request) -> bool:
            ...
            if not tok:
                tok = req.cookies.get("jarvis_token", "").strip()
        ...
        async def index():
            # Auth is handled client-side via sessionStorage/localStorage/URL bearer token.
            html = (self._app_html
        ...
        async def login(req: Request):
            ...
            # 1. Reuse existing active session if available
            if entered in self._active_key_sessions and self._active_key_sessions[entered].get("expires", 0) > now:
                sess = self._active_key_sessions[entered]
                return JSONResponse({
                    "ok": True,
                    "token": sess["token"],
                    "key": entered,
                    "device_token": sess.get("device_token", ""),
                })

            # 2. Validate pending key
            if entered in self._pending_keys and self._pending_keys[entered] > now:
                expires_at = self._pending_keys[entered]
                ...
                self._active_key_sessions[entered] = {
                    "token": tok,
        ...
        async def auto_login(key: str = ""):
            """QR code target — validates key, creates session, redirects phone safely."""
            ...
            # 1. Check if an active session was already created for this key (handles mobile prefetch/multi-scan)
            if key in self._active_key_sessions and self._active_key_sessions[key].get("expires", 0) > now:
                sess = self._active_key_sessions[key]
                tok = sess["token"]
                dev_tok = sess.get("device_token", "")
            elif key in self._pending_keys and self._pending_keys[key] > now:
                expires_at = self._pending_keys[key]
                tok = secrets.token_urlsafe(32)
                dev_tok = secrets.token_urlsafe(32)
                self._tokens.add(tok)
...
<p>Press <strong style="color:#dde3ed">Remote Control</strong> in ARC to get a new QR code.</p>
</div></body></html>""")
...
<style>
  body{{background:#07090f;color:#dde3ed;font-family:sans-serif;
       display:flex;align-items:center;justify-content:center;height:100vh;margin:0;text-align:center}}
  p{{color:#00d2ff;font-size:16px;letter-spacing:1px}}
</style></head>
<body>
<script>
  try {{
    ...
    document.cookie = 'jarvis_token={tok}; path=/; max-age=86400; SameSite=Lax';
    document.cookie = 'jarvis_key={key}; path=/; max-age=86400; SameSite=Lax';
  }} catch(e) {{}}
  location.replace('/?token={tok}&key={key}');
</script>
<p>Connecting to ARC…</p>
</body></html>""")
        ...
        async def stop_ep(req: Request):
            ...
            if self._interrupt_callback:
                try:
                    ...
                    pass
            asyncio.create_task(self.broadcast(
                {"type": "sys", "text": "🛑 Emergency stop triggered from remote dashboard."}
            ))
        ...
        async def get_gpu_telemetry(req: Request):
            try:
                from core.gpu_accelerator import get_vram_telemetry
                return JSONResponse(get_vram_telemetry())
            except Exception as e:
                return JSONResponse({"error": str(e), "active": False})

        @app.get("/api/bandwidth")
        async def get_bw_metrics(req: Request):
            from core.bandwidth_monitor import get_metrics
            return JSONResponse(get_metrics())

        @app.get("/api/camera/status")
        async def camera_status_ep(req: Request):
            from core.device_service import list_connected_cameras, get_active_camera_index, get_active_camera_name
            from actions.gesture_control import GestureController
            ctrl = GestureController.get_instance()
            return JSONResponse({
                "cameras": list_connected_cameras(),
                "active_index": get_active_camera_index(),
                "active_name": get_active_camera_name(),
                "gesture_active": ctrl.is_running(),
                "last_gesture": ctrl._last_gesture,
            })

        @app.post("/api/camera/switch")
        async def camera_switch_ep(req: Request):
            if not _auth(req):
                return JSONResponse({"error": "Unauthorized"}, status_code=401)
            try:
                body = await req.json()
                ...
                body = {}
            target = body.get("camera", 0)
            from core.device_service import set_active_camera
            ok, msg = set_active_camera(target)
            return JSONResponse({"ok": ok, "message": msg})

        @app.post("/api/gesture/toggle")
        async def gesture_toggle_ep(req: Request):
            if not _auth(req):
                return JSONResponse({"error": "Unauthorized"}, status_code=401)
            from actions.gesture_control import GestureController
            ctrl = GestureController.get_instance()
            ...
            asyncio.create_task(self.broadcast({
                "type": "gesture",
                "active": active,
                "gesture": ctrl._last_gesture,
            }))
            return JSONResponse({"ok": ok, "active": active, "message": msg})
        ...
        async def ws_ep(websocket: WebSocket, token: str = ""):
            ...
            # Send current network / bandwidth status immediately
            try:
                from core.bandwidth_monitor import get_bandwidth_state, get_metrics
                await websocket.send_json({
                    "type": "bandwidth_state",
            ...
            try:
                while True:
                    ...
                    if msg_type in ("ping", "bandwidth_check"):
                        from core.bandwidth_monitor import get_bandwidth_state, get_metrics
                        await websocket.send_json({
                            "type": "pong",
        ...
        # Register plugin extension routes (e.g. screen_mirror)
        for fn in _dashboard_extension_routes:
            try:
                fn(app, self)
            except Exception as e:
                print(f"[Dashboard] Extension route build error: {e}")
    ...
    async def serve(self) -> None:
        ...
        print("[Dashboard] Press 'Remote Control' in ARC UI to get the QR code.")
        await uvicorn.Server(cfg).serve()
